...
const calculateStreak = async (habitId, userId) => {
  const logs = await HabitLog.find({ habitId, userId, completed: true }).sort({
    date: -1,
  });
...
router.post("/", authMiddleware, async (req, res) => {
  try {
    const { name, description, category, color, frequency, target, reminder } =
      req.body;
  ...
  try {
    ...
    if (completed) {
      ...
      const completionRate =
        allLogs.length > 0
          ? Math.round((completedCount / allLogs.length) * 100)
          : 0;
      ...
      const userCurrentStreak = Math.max(
        ...allHabits.map((h) =>
          h._id.toString() === habitId ? streak : h.stats?.currentStreak || 0,
        ),
      );
      const userLongestStreak = Math.max(
        ...allHabits.map((h) =>
          h._id.toString() === habitId
    ...
    res
      .status(500)
      .json({ error: "Failed to log habit", details: error.message });
  }
});
